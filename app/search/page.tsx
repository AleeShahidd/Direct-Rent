...
function SearchPageContent() {
  ...
  const searchProperties = useCallback(async (searchFilters: PropertySearchFilters) => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          landlord:users!landlord_id(full_name, email, phone)
        `)
        .eq('is_active', true)
        .eq('is_verified', true)
      ...
      if (searchFilters.property_type && searchFilters.property_type.length > 0) {
        query = query.in('property_type', searchFilters.property_type)
      }
      if (searchFilters.min_price) {
        query = query.gte('price_per_month', searchFilters.min_price)
      }
      if (searchFilters.max_price) {
        query = query.lte('price_per_month', searchFilters.max_price)
      }
      ...
      if (searchFilters.furnishing_status && searchFilters.furnishing_status.length > 0) {
        query = query.in('furnishing_status', searchFilters.furnishing_status)
      }
      if (searchFilters.council_tax_band && searchFilters.council_tax_band.length > 0) {
      ...
      // Apply pagination
      const from = ((searchFilters.page || 1) - 1) * (searchFilters.limit || 12)
  ...
  const handleFiltersChange = (newFilters: PropertySearchFilters) => {
    ...
    if (updatedFilters.postcode) params.set('location', updatedFilters.postcode)
    if (updatedFilters.city && !updatedFilters.postcode) params.set('location', updatedFilters.city)
    if (updatedFilters.property_type?.length) params.set('property_type', updatedFilters.property_type[0])
    ...
    if (updatedFilters.min_bedrooms) params.set('min_bedrooms', updatedFilters.min_bedrooms.toString())
    if (updatedFilters.max_bedrooms) params.set('max_bedrooms', updatedFilters.max_bedrooms.toString())
